  ...
  async execute(oldState, newState) {
    ...
    if (newState.channelId && oldState.channelId !== newState.channelId) {
      try {
        ...
        if (rows.length > 0) {
          console.log(
            `User ${newState.member.displayName} joined main channel (${newState.channelId}). Creating sub-channel...`
          );

          // Get the main channel object (to inherit its category, for example)
          const mainChannel = newState.guild.channels.cache.get(
            newState.channelId
          );
          if (!mainChannel) {
          ...
          const existingChannels = newState.guild.channels.cache.filter(
            (ch) => {
          ...
          // Set the new channel name to the user's display name
          const sanitizedDisplayName = newState.member.displayName
            .replace(/[^a-zA-Z0-9-_ ]/g, "")
            .trim();
          const newChannelName = `${sanitizedDisplayName}'s Channel`;
          ...
          console.log(
            `Created and stored new sub-channel (${newChannel.id}) for ${newState.member.displayName} with name "${newChannelName}"`
          );

          // Move the member into their new channel
          await newState.member.voice.setChannel(newChannel);
          console.log(
            `Moved ${newState.member.displayName} to their sub-channel.`
          );
        }
      } catch (error) {
    ...
    if (oldState.channelId) {
      const createdChannel = newState.client.voiceChannelOwners.get(
        oldState.channelId
      );
      if (createdChannel) {
        ...
        if (
          ...
          try {
            ...
            console.log(
              `Deleted channel ${oldState.channelId} and removed it from the DB`
            );
          } catch (error) {
